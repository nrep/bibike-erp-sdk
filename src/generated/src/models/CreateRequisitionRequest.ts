/* tslint:disable */
/* eslint-disable */
/**
 * Bibike ERP API
 * REST API for Bibike ERP - a modular Enterprise Resource Planning system for SMEs in East Africa.  ## Authentication All endpoints (except auth/login and health) require Bearer token authentication. Include the token in the Authorization header: `Authorization: Bearer <token>`  ## Multi-Tenancy All data is scoped to the authenticated user\'s organization. Cross-organization access is not permitted.  ## Versioning API version can be specified via: - Query parameter: `?v=1` - Header: `X-API-Version: 1`  Current supported versions: **v1** (current), **v2** (preview)  Response headers include: - `X-API-Version`: The version used for this request - `X-API-Min-Version`: Minimum supported version - `X-API-Max-Version`: Maximum supported version - `X-API-Deprecated`: Present if using a deprecated version - `X-API-Sunset`: Date when deprecated version will be removed  ## Idempotency For POST/PUT/DELETE requests, include `Idempotency-Key` header to prevent duplicate operations on retries.  ## Rate Limiting Default: 60 requests/minute per token. Custom limits available for integration tokens.  Response headers include: - `X-RateLimit-Limit`: Maximum requests allowed per minute - `X-RateLimit-Remaining`: Requests remaining in current window - `X-RateLimit-Reset`: Unix timestamp when the window resets - `X-RateLimit-Used`: Requests used in current window  When rate limited, you\'ll receive a `429 Too Many Requests` response with a `Retry-After` header. 
 *
 * The version of the OpenAPI document: 1.2.0
 * Contact: support@bibike.fivorana.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { CreateRequisitionRequestItemsInner } from './CreateRequisitionRequestItemsInner';
import {
    CreateRequisitionRequestItemsInnerFromJSON,
    CreateRequisitionRequestItemsInnerFromJSONTyped,
    CreateRequisitionRequestItemsInnerToJSON,
} from './CreateRequisitionRequestItemsInner';

/**
 * 
 * @export
 * @interface CreateRequisitionRequest
 */
export interface CreateRequisitionRequest {
    /**
     * 
     * @type {number}
     * @memberof CreateRequisitionRequest
     */
    departmentId?: number;
    /**
     * 
     * @type {string}
     * @memberof CreateRequisitionRequest
     */
    purpose?: string;
    /**
     * 
     * @type {string}
     * @memberof CreateRequisitionRequest
     */
    priority?: CreateRequisitionRequestPriorityEnum;
    /**
     * 
     * @type {Date}
     * @memberof CreateRequisitionRequest
     */
    requiredDate?: Date;
    /**
     * 
     * @type {string}
     * @memberof CreateRequisitionRequest
     */
    notes?: string;
    /**
     * 
     * @type {Array<CreateRequisitionRequestItemsInner>}
     * @memberof CreateRequisitionRequest
     */
    items: Array<CreateRequisitionRequestItemsInner>;
}


/**
 * @export
 */
export const CreateRequisitionRequestPriorityEnum = {
    Low: 'low',
    Normal: 'normal',
    High: 'high',
    Urgent: 'urgent'
} as const;
export type CreateRequisitionRequestPriorityEnum = typeof CreateRequisitionRequestPriorityEnum[keyof typeof CreateRequisitionRequestPriorityEnum];


/**
 * Check if a given object implements the CreateRequisitionRequest interface.
 */
export function instanceOfCreateRequisitionRequest(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "items" in value;

    return isInstance;
}

export function CreateRequisitionRequestFromJSON(json: any): CreateRequisitionRequest {
    return CreateRequisitionRequestFromJSONTyped(json, false);
}

export function CreateRequisitionRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): CreateRequisitionRequest {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'departmentId': !exists(json, 'department_id') ? undefined : json['department_id'],
        'purpose': !exists(json, 'purpose') ? undefined : json['purpose'],
        'priority': !exists(json, 'priority') ? undefined : json['priority'],
        'requiredDate': !exists(json, 'required_date') ? undefined : (new Date(json['required_date'])),
        'notes': !exists(json, 'notes') ? undefined : json['notes'],
        'items': ((json['items'] as Array<any>).map(CreateRequisitionRequestItemsInnerFromJSON)),
    };
}

export function CreateRequisitionRequestToJSON(value?: CreateRequisitionRequest | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'department_id': value.departmentId,
        'purpose': value.purpose,
        'priority': value.priority,
        'required_date': value.requiredDate === undefined ? undefined : (value.requiredDate.toISOString().substring(0,10)),
        'notes': value.notes,
        'items': ((value.items as Array<any>).map(CreateRequisitionRequestItemsInnerToJSON)),
    };
}

